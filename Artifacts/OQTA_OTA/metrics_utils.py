...
def test(f_label, uf_label, unprivileged_groups, privileged_groups, dataset, model, thresh_arr, metric_arrs):
    """
    Evaluate a model across a range of thresholds using fairness metrics.
    Supports both sklearn and PyTorch-based classifiers.
    """
    print("=======================================================================")
    print("TEST")

    try:
        # Case 1: Torch classifier
        ...
        # Case 2: sklearn classifier
        else:
            print("Checking predict_proba")
    ...
    # Case 3: fallback method (e.g., Fairlearn's _pmf_predict)
    ...
    for thresh in thresh_arr:
        ...
        # Count group-wise predictions
        unpriv_indices = np.where(dataset.features[:, sensitive_feature_index] == 0)
        priv_indices = np.where(dataset.features[:, sensitive_feature_index] == 1)
        ...
        # Disparate Impact (normalized)
        disp_impact = metric.disparate_impact()
        if np.isnan(disp_impact) or disp_impact == 0:
            disp_impact = np.mean(metric_arrs['disp_imp']) if metric_arrs['disp_imp'] else 1
...
def get_test_metrics_for_cpp(DATASET, target_dataset, reference_dataset, dataset_orig_train, dataset_orig_val, dataset_orig_test, model_type, 
                     test_metrics, mia_metrics, ATTACK, log_type, f_label=None, uf_label=None, 
                     unprivileged_groups=None, privileged_groups=None, THRESH_ARR=None, DISPLAY=None, SCALER=None):
    dataset = dataset_orig_train
