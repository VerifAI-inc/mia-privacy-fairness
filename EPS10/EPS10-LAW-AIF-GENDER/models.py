...
class DPRF(BaseModel):
    ...
    def train (self, DATASET, dataset_train, SCALER, ATTACK):
        ...
        if ATTACK == "mia1":
            if SCALER:
                model = make_pipeline(StandardScaler(), dp.RandomForestClassifier(random_state=1, epsilon=10, bounds=(0,1)))
            else:
                model = make_pipeline(dp.RandomForestClassifier(random_state=1, epsilon=10, bounds=(0,1)))
