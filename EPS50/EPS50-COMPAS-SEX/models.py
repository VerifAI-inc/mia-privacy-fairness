...
class DPLRModel(BaseModel):
    ...
    def train (self, dataset_train, SCALER, ATTACK):
        ...
        if ATTACK == "mia1":
            ...
            model = dp.LogisticRegression(solver='liblinear', random_state=1, epsilon=50, bounds=(lower_bounds,upper_bounds))
...
class DPRF(BaseModel):
    ...
    def train (self, DATASET, dataset_train, SCALER, ATTACK):
        ...
        if ATTACK == "mia1":
            if SCALER:
                model = make_pipeline(StandardScaler(), dp.RandomForestClassifier(random_state=1, epsilon=50, bounds=(0,1)))
            else:
                model = make_pipeline(dp.RandomForestClassifier(random_state=1, epsilon=50, bounds=(0,1)))
            ...
            if DATASET == "bank" or DATASET.startswith("german") or DATASET == "meps19":
                classifier = dp.RandomForestClassifier(random_state=1,  epsilon=50, bounds=(lower_bounds, upper_bounds), max_depth=15)
            elif DATASET.startswith("compas"):
                classifier = dp.RandomForestClassifier(random_state=1,  epsilon=50, bounds=(lower_bounds, upper_bounds), max_depth=6)
            elif DATASET == "law_sex" or DATASET == "law_race":
                classifier = dp.RandomForestClassifier(random_state=1,  epsilon=50, bounds=(lower_bounds, upper_bounds), max_depth=7)
            elif DATASET.startswith("law"):
                classifier = dp.RandomForestClassifier(random_state=1,  epsilon=50, bounds=(lower_bounds, upper_bounds), max_depth=3)
