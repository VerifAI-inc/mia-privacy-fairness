...
class PopulationMetric(Metric):
    ...
    def run_metric(self, fpr_tolerance_rate_list=None) -> List[MetricResult]:
        ...
        # Calculate TPR, TNR, accuracy for each threshold for the current group
        best_accuracy = 0
        best_tpr = 0
        best_tnr = 0
        best_index = -1

        for idx in range(num_threshold):
            tp = np.sum(member_preds[idx])
            tn = np.sum(~non_member_preds[idx])
            fp = np.sum(non_member_preds[idx])
            fn = np.sum(~member_preds[idx])
        ...
        # Store the best metrics for the group
        population_metrics = {
            "tpr": best_tpr,
            "tnr": best_tnr,
            "accuracy": best_accuracy,
            "best_threshold_index": best_index,
            "member_pred": member_preds,
            "non_member_pred": non_member_preds,
            "thresholds": thresholds,
            "member_signals": self.member_signals,
            "non_member_signals": self.non_member_signals,
            "tprs": np.array(tprs),
            "fprs": np.array(fprs)
        }
        
        return metric_result, population_metrics
...
class GroupPopulationMetric(Metric):
    ...
    def run_metric(self, fpr_tolerance_rate_list=None) -> List[MetricResult]:
        ...
        # New dictionaries to store group-specific predictions and metrics
        group_metrics = {}
        ...
        for g in np.unique(self.reference_groups):
            ...
            for idx in range(num_threshold):
                tp = np.sum(member_pred[idx])
                tn = np.sum(~non_member_pred[idx])
                fp = np.sum(non_member_pred[idx])
                fn = np.sum(~member_pred[idx])
            ...
            group_metrics[g] = {
                ...
                "non_member_pred": non_member_pred,
                "thresholds": thresholds,
                "member_signals": group_member_signals,
                "non_member_signals": group_non_member_signals,
                "tprs": np.array(tprs),
                "fprs": np.array(fprs)
            }
        ...
        return metric_result, group_metrics